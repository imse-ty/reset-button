...
export const KEYS = {
  ...
  tz: 'reset:tz', // IANA timezone of the last device that turned notifications on
  nudged: 'reset:nudged' // ms timestamp of the last "time to start" reminder
};

// One hourly QStash schedule; /api/remind decides in local time whether to send anything.
// A fixed scheduleId makes this an upsert, so calling it on every subscribe is safe.
export function ensureReminders(req) {
  return queue().schedules.create({
    destination: `${appUrl(req)}/api/remind`,
    cron: '0 * * * *',
    scheduleId: 'reset-remind',
    retries: 0
  });
}
